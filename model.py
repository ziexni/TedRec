...
class MoEAdaptorLayer(nn.Module):
    ...
    def __init__(self, n_exps, layers, dropout=0.0, max_seq_length=50, noise=True):
        ...
        self.experts = nn.ModuleList([
            DTRLayer(layers[0], layers[1], dropout, max_seq_length) 
    ...
    def forward(self, x):
        gates = self.noisy_top_k_gating(x, self.training)  # (B, L, n_E)
        expert_outputs = [self.experts[i](x).unsqueeze(-2) for i in range(self.n_exps)]  # [(B, L, 1, D)]
...
class TedRec(SASRec):
    ...
    def contextual_convolution(self, item_emb, feature_emb):
        ...
        item_conv = torch.fft.irfft(item_fft * complex_weight, n=feature_emb.shape[1], dim=1, norm='ortho')
        ...
        # CRITICAL: 2x scaling from original TedRec
        contextual_emb = 2 * (item_conv * torch.sigmoid(item_gate_w) + fusion_conv * torch.sigmoid(fusion_gate_w))
        return contextual_emb
    
    def forward(self, item_seq, item_seq_len):
        """Standard forward for RecBole compatibility"""
        # 내부에서 item_emb 계산
        item_emb_list = self.moe_adaptor(self.plm_embedding(item_seq))
        # 실제 forward 로직 실행
        return self._forward_with_moe(item_seq, item_emb_list, item_seq_len)
    
    def _forward_with_moe(self, item_seq, item_emb, item_seq_len):
        """Internal forward with MoE features"""
    ...
    def calculate_loss(self, interaction):
        ...
        item_emb_list = self.moe_adaptor(self.plm_embedding(item_seq))
        seq_output = self._forward_with_moe(item_seq, item_emb_list, item_seq_len)
